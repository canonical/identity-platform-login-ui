---
applyTo: "ui/**"
description: "Frontend conventions for Next.js pages, Ory Flow/Node components, and Vanilla Framework styling"
---
...
`next.config.js` configures rewrites **only when `DEV=true`** — always start the dev server as `DEV=true npm run dev`:

| Incoming request (Next.js :3001) | Forwarded to         |
|----------------------------------|----------------------|
| `/api/*`                         | Go backend `:4455`   |
...
**Without `DEV=true`** (and in all production deployments) there are no rewrites — Next.js emits a static export (`ui/dist/`) that the Go binary serves directly. Never use `next start`; this app uses `output: 'export'`.

The browser must always be opened at `http://localhost` (Traefik on port 80), not at `:3001` directly.
...
- All page components are `NextPage` or `FC<Props>`; never use class components unless strictly required.
