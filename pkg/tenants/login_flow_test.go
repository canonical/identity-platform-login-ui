...
func TestInjectTenantPayload(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "identifier-first method",
			body: func() *client.UpdateLoginFlowBody {
				b := client.UpdateLoginFlowWithIdentifierFirstMethodAsUpdateLoginFlowBody(
					client.NewUpdateLoginFlowWithIdentifierFirstMethod("user@example.com", "identifier_first"),
				)
				return &b
			},
			expected: true,
...
func getTransientPayload(body *client.UpdateLoginFlowBody) map[string]interface{} {
	...
	case body.UpdateLoginFlowWithIdentifierFirstMethod != nil:
		p, ok := body.UpdateLoginFlowWithIdentifierFirstMethod.GetTransientPayloadOk()
		if !ok || p == nil {
			return nil
		}
		return p
