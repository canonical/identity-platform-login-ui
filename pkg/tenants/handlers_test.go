...
func TestHandleResolveTenantMissingFlow(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	body, _ := json.Marshal(resolveRequest{LoginChallenge: "lc-1"})
	req := httptest.NewRequest(http.MethodPost, "/api/v0/tenants/resolve", bytes.NewReader(body))
...
func TestHandleResolveTenantMissingLoginChallenge(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	body, _ := json.Marshal(resolveRequest{Flow: "flow-1"})
	req := httptest.NewRequest(http.MethodPost, "/api/v0/tenants/resolve", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
...
func TestHandleResolveTenantNoTenants(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// 0-tenant case: handler stores the no-tenant sentinel (sets LoginChallengeHash
	// in the cookie) but returns {} so the frontend's followData() keeps the user
	// on the same Kratos flow and proceeds directly to the 1FA step.
	mockService.EXPECT().
		ResolveTenant(gomock.Any(), "flow-1", "lc-1", gomock.Any()).
		Return(&ResolveResult{}, nil)
	mockStorer.EXPECT().StoreTenant(gomock.Any(), gomock.Any(), cookies.NoTenantAvailable, "lc-1").Return(nil)

	mux := chi.NewMux()
	NewAPI(mockService, mockStorer, nil, "http://localhost", mockTracer, mockLogger).RegisterEndpoints(mux)

	body, _ := json.Marshal(resolveRequest{Flow: "flow-1", LoginChallenge: "lc-1"})
	req := httptest.NewRequest(http.MethodPost, "/api/v0/tenants/resolve", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	...
	if result.RedirectTo != "" {
		t.Fatalf("expected empty redirect_to for no-tenant case, got %q", result.RedirectTo)
	}
}

func TestHandleResolveTenantRedirect(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// N-tenant case: service returns a redirect to the selection page.
	mockService.EXPECT().
		ResolveTenant(gomock.Any(), "flow-1", "lc-1", gomock.Any()).
		Return(&ResolveResult{RedirectTo: "/ui/select_tenant?flow=flow-1&login_challenge=lc-1"}, nil)
	...
	if result.RedirectTo == "" {
		t.Fatal("expected non-empty redirect_to")
	}
}

func TestHandleResolveTenantSingleTenant(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// Single-tenant: service returns TenantID; handler must store + return login redirect.
	mockService.EXPECT().
		ResolveTenant(gomock.Any(), "flow-1", "lc-1", gomock.Any()).
		Return(&ResolveResult{TenantID: "t1"}, nil)
	mockStorer.EXPECT().StoreTenant(gomock.Any(), gomock.Any(), "t1", "lc-1").Return(nil)
	...
	if result.RedirectTo == "" {
		t.Fatal("expected redirect_to to be set for single-tenant case")
	}
	if result.TenantID != "" {
		t.Fatalf("TenantID must not be serialised to JSON, got %q", result.TenantID)
	}
}
...
func TestHandleResolveTenantServiceError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
