...
`MULTI_TENANCY_ENABLED=true`. When disabled, tenant-related endpoints are not available,
so the frontend must behave identically to a non-tenant deployment and must not call
tenant APIs.
